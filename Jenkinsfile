        ...
        try {
            stage('Build') {
                for(def component : configuration.components ) {
                    ...
                    bat "\"${tool 'msbuild'}\" $solution /p:DeployOnBuild=true;DeployTarget=Package /p:Configuration=Release; /p:Platform=\"Any CPU\" /p:ProductVersion=1.0.0.${env.BUILD_NUMBER}"
                }
             }
